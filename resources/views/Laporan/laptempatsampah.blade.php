...
<main id="main" class="main">
    <div>
        <h1>Laporan Tempat Sampah</h1> <!-- Center-aligned title -->
        <div class="print-button">
        <a  href="{{ url('/cetaklaptempatsampah') }}" class="btn btn-primary"> Cetak 
        </a>
        <div class="sensor-data-container">
            <table class="sensor-data-table">
                ...
                <tbody>
                    @php
                    $maxRows = count($tpsampah);
                    @endphp
                    @for ($i = 0; $i < $maxRows; $i++)
                    <tr>
                        <td>{{ $i + 1 }}</td>
                        <td>{{ $tpsampah[$i]->created_at ?? '' }}</td>
                        <td>{{ $tpsampah[$i]->nilai_tinggi_sampah ?? '' }}</td>
                        <td>{{ $tpsampah[$i]->keterangan ?? '' }}</td>
                    </tr>
                    @endfor
                </tbody>
            </table>
