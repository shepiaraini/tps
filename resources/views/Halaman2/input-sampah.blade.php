...
<main id="main" class="main">
    <div>
        <h1>Ketinggian Sampah</h1> <!-- Center-aligned title -->
                <a href="{{ url('tambah-sampah') }}" class="btn btn-primary">Tambah</a>
              </div>
            </div><!-- /.card-header -->
            <div class="card-body">
              <table class="table table-bordered">
                ...
                <tbody>
                  @foreach ($dtambahsampah as $item)
                  <tr>
                    <td>{{ $loop->iteration }}</td>
                    <td>{{ $item->tanggal }}</td>
                    <td>{{ $item->ketinggian_sampah }}</td>
                    <td>
                     <a href="{{ url('edit-sampah', $item->id_input_sampah) }}" class="btn btn-warning">
                     <i class="fas fa-edit"></i>
                    </a>
                     <a href="{{ url('delete-sampah', $item->id_input_sampah) }}" class="btn btn-danger">
